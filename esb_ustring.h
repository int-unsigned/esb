...
#pragma once
#ifndef ESB_USTRING_H_
#define ESB_USTRING_H_
...
namespace esb {
	...
	constexpr inline UCharType to_upcase_ru(UCharType wch_) {
		...
		return (wch_ == letters_val::ru_ё) ? static_cast<UCharType>(letters_val::ru_Ё) :
			(wch_ >= letters_val::ru_а && wch_ <= letters_val::ru_я) ? static_cast<UCharType>(wch_ - static_cast<UCharType>(letters_val::ru_а - letters_val::ru_А)) :
			wch_;
	}
	template<class UCharType>
	...
	constexpr bool сstr_equal(UStrIteratorType1 sit1_, UStrIteratorType2 sit2_) {
